...
self.addEventListener('fetch', event => {
    ...
    if (theURL.endsWith(".html"))
        process = true;
    if (new RegExp(`/r/.*/t/.*`).test(theURL))
        process = true;
    if (new RegExp(`/r/.*/s/.*`).test(theURL))
        process = true;
    if (new RegExp(`/r/.*/backup.*`).test(theURL))
        process = false;
    if (theURL.startsWith(`/large`))
        process = false;
    if (new RegExp(`/r/.*/sync.*`).test(theURL))
        process = false;
    if (new RegExp(`/r/.*/t/.*\.json$`).test(theURL))
        process = false;
...
/******/ })()
;
//# sourceMappingURL=sw-library-0.16.53.js.map